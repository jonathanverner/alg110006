def build_automaton(retezec):
    """ Vrátí automat rozpoznávající řetězec retezec.
        ...
        Funkce ve skutečnosti vrátí seznam stavů, kde stav stav s
        indexem 0 je počáteční a stav poslední stav je koncovým
        stavem. Každý stav je seznamem šipek, které z něj vedou,
        t.j. dvojice (znak, index_následujícího_stavu_pro_daný_znak)
    """
    ...
    for c in retezec:
        ...
        # Přidáme zpětné šipky, t.j. zkopírujeme šipky z pomocného stavu
        # do aktuálního stavu. Pokud z pomocného stavu vede šipka
        # označená znakem c pak příslušně aktualizujeme pomocný stav,
        # jinak nastavíme pomocný stav na 0
        next_help_state = A[0]
...
def optimalfind(text, s):
    A = build_automaton(s)
    c_state = A[0]             # aktuální stav nastavíme na počáteční stav
                               # automatu
    ...
    for c in text:
        pos = pos+1
        # Najdi šipku odpovídající načtenému písmenu. Pokud taková je,
        # přesuň se do odpovídajícího stavu, jinak se přesuň do
        # počátečního stavu
        move_to_zero = True
